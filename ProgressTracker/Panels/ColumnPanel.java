...
public class ColumnPanel extends JPanel{
    ...
    public ColumnPanel(MyFrame myFrame, Records records, int columnNumber, int noteCount){
        ...
        for(int counter = 0; counter < noteCount; counter++){
            ...
            notePanels.get(counter).getTitle().addActionListener(myFrame);
            notePanels.get(counter).getContents().addActionListener(myFrame);
            notePanels.get(counter).getUser().addActionListener(myFrame);
